...
node = '1'
core = "1" 
out = "central_carbon_ps_out"
err = "central_carbon_ps_err"
...
for dispatch in range(dispatch_num):
    ...
    slurm += "max=" + str((dispatch + 1) * loop_num - 1) + "\nfor i in `seq " + str(dispatch * loop_num) + " $max`\ndo\n    python scripts/main.py -config=cyano_small_system/central_carbon_ps/central_carbon_ps.json -i=$i\ndone\n"
